...
- **系统事件日志查看与分析**：独立「系统日志」标签页，覆盖 Application / System / Security 等常用通道；
  ...
  读取在后台线程进行，界面不阻塞。
- **筛选自动刷新**：通道 / 级别 / 时间范围 / 事件 ID / 来源 / 最多取回 任一改动后**无需再点按钮**，
  防抖合并后自动重新读盘（下拉与复选框点完约 0.35 秒生效，输入框、时间框停止输入约 0.9 秒生效，
  连续操作只算一次查询）；读盘途中改条件会在本轮读完后立即按最新条件补查，操作不会丢。
  刷新完成后状态栏以「⟳ 已按新筛选条件自动刷新」点明这是筛选变更触发的。若要一次填好几个字段
  再执行，可取消勾选「自动刷新」回到手动「查询 / 刷新」模式（关键字始终是输入即时过滤，不读盘）。
- **日志详情区可拖拽放大**：列表与统计区之间是可抓取的分隔条（加宽 + 悬停高亮），可一键「收起统计区」
  把整个高度让给日志列表，列表字号支持 A- / A+ 缩放；单条详情窗口可自由拉伸 / 最大化，内部
  「字段 / 描述 / 原始 XML」三块也能分别拖大，长描述不再被限高在小框里滚动。
- **开机时间与运行时长**：一键「开机与运行」筛出 System 通道里的开机 / 关机 / 运行时长事件
  （EventLog 6005/6006/6008/6009/6013、Kernel-General 12/13、User32 1074），并给出摘要条
  「最近开机 … · 当前状态 … · 本次已运行 …」；6013 的原始字段会被翻译成
  「系统已运行 7586 秒（≈2 小时 6 分）」这样的可读文本。
...
- **分析统计层（`eventlog_stats.py`）**：纯函数，对取回的记录做级别数量与占比、按事件 ID / 来源 / 时间段的聚合，识别高频错误与异常趋势；另含开机 / 关机 / 运行时长摘要（`boot_summary`，见下）。
...
| 位置 | 交互 | 说明 |
|---|---|---|
| 列表 ↔ 统计区 | 拖动分隔条 | 手柄加宽到 9px 并悬停高亮（原为 1px 无提示）；两侧均设最小高度，不会被误拖成 0 |
| 分页栏右侧 | 「收起统计区」 | 折叠统计/分析区，整个高度让给日志列表（再次点击还原折叠前的比例） |
| 分页栏右侧 | `A-` / `A+` | 缩放列表字号，行高同步放大（档位有上下限，不会越界） |
| 列表行 | 双击 / 右键 | 打开详情；右键菜单另有「复制描述」「复制原始 XML」 |
| 详情窗口 | 拉伸 / 最大化 | 默认 940×720、带右下角手柄与最大化按钮，尺寸在同一会话内记忆 |
| 详情窗口内 | 拖动分隔条 | 「字段 / 描述 / 原始 XML」三块可分别拖大（描述不再被 `setMaximumHeight(140)` 限高） |
| 详情窗口内 | `A-` / `A+`（或 Ctrl+±） | 缩放正文（字段表 / 描述 / XML）字号 |

#### 开机时间与运行时长

Windows 把这些信息写在 System 通道的固定事件里，但它们都是「信息」级别、混在数千条日志中，手工设筛选很麻烦，所以提供了 **「开机与运行」一键预设**（位于筛选栏按钮行，会填好来源、事件 ID 与「近30天」并立即查询）：

| 事件 | 含义 |
|---|---|
| `EventLog` 6005 / 6006 | 事件日志服务启动（开机标记）/ 停止（正常关机） |
| `EventLog` 6008 | 上一次关机是意外的 |
| `EventLog` 6009 | 启动版本信息（OS 版本 / Build） |
| `EventLog` 6013 | 系统已运行时间 —— EventData 第 5 个字段即「已运行秒数」 |
| `Microsoft-Windows-Kernel-General` 12 / 13 | OS 启动（`StartTime` = 开机时刻）/ 关闭（`StopTime`） |
| `User32` 1074 | 关机 / 重启由哪个进程发起 |

命中这些事件时：结果上方出现摘要条（「最近开机 … · 当前状态：运行中 / 已关机 · 本次已运行 ≈4 小时」），列表「描述」列给出 `【系统已运行 7586 秒（≈2 小时 6 分）】` 这类可读前缀，详情里多一行「事件含义」。运行时长有两路来源，可互相校验：6013 日志自报值，以及由最近开机时刻推断的「本次已运行」（本机实测与 `GetTickCount64` 相差 < 1 分钟）。
...
python check_eventlog.py        # 系统日志模块（XPath 构造、XML 解析、统计聚合、开机/运行时长摘要、面板分页/筛选/导出、详情区布局与缩放、标签页索引隔离）
